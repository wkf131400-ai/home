...
export const CustomerPool: React.FC<CustomerPoolProps> = ({
  customers,
  onSelectCustomer,
  onAddCustomer,
  onBatchAssign,
  ...
  const [followUpStatus, setFollowUpStatus] = useState('全部');
  const [activeTab, setActiveTab] = useState<'全部' | '资源客户' | '意向客户' | '丢单客户'>('全部');
  ...
  // Filter pool customers (where isPool is true or shown in pool)
  const poolCustomers = customers.filter((c) => {
    // Tab filter
    ...
    if (followUpStatus !== '全部' && c.followUpStatus !== followUpStatus) {
      return false;
    }

    // More filters
    if (sourceFilter !== '全部' && c.source !== sourceFilter) return false;
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === poolCustomers.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(poolCustomers.map((c) => c.id));
    }
  };
  ...
  const handleExport = () => {
    const csvContent =
      ...
      ['序号,客户姓名,项目名称,客户电话,跟进状态,客户状态,交付状态,价格等级,创建人,创建时间,更新时间']
        .concat(
          poolCustomers.map(
            (c, i) =>
              `${i + 1},${c.name},${c.projectName},${c.phone},${c.followUpStatus},${c.status},${c.deliveryStatus},${c.priceGrade},${c.creator},${c.createdAt},${c.updatedAt}`
          )
        )
        .join('\n');
    ...
    link.setAttribute('download', `客户池列表_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      {/* 1. Search & Filter Bar (Matching Image 1) */}
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs space-y-3">
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          <div className="w-40">
            ...
            >
              <option value="全部">请选择跟进状态</option>
              <option value="未跟进">未跟进</option>
              <option value="08-18 16:26">08-18 16:26</option>
              <option value="已出方案">已出方案</option>
        ...
        {showMoreFilters && (
          <div className="pt-3 border-t border-slate-100 grid grid-cols-4 gap-4 text-xs">
            <div>
              <label className="block text-slate-500 mb-1">客户来源:</label>
              <select
                value={sourceFilter}
            ...
            <div>
              <label className="block text-slate-500 mb-1">客户级别:</label>
              <select
                value={levelFilter}
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-2xs overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-2.5 border-b border-slate-200 bg-white">
          ...
          <div className="flex space-x-6 text-xs font-bold">
            {(['全部', '资源客户', '意向客户', '丢单客户'] as const).map((tab) => (
              <button
                key={tab}
            ...
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold">
              <tr>
                <th className="py-3 px-3 w-10 text-center">
                  <input
                    type="checkbox"
                    checked={poolCustomers.length > 0 && selectedIds.length === poolCustomers.length}
                ...
                <th className="py-3 px-4 text-slate-600">更新时间</th>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-slate-200 bg-slate-50 text-xs text-slate-500">
          <div>
            共 <span className="font-bold text-slate-800">{poolCustomers.length}</span> 条
            {selectedIds.length > 0 && (
